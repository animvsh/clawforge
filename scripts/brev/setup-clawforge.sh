...
install_node_20() {
  ...
  if ! have curl; then
    fail "curl is required to install Node.js 20 on this Brev VM."
  fi
  if ! have sudo; then
    fail "sudo is required to install Node.js 20 on this Brev VM."
  fi
...
if ! have git; then
  fail "git is required on the Brev instance."
fi
...
if ! have npm; then
  fail "npm is required after Node.js installation."
fi
...
if have docker; then
  ...
  warn "Docker was not found. NemoClaw/OpenShell onboarding may install or require it."
fi

if [[ "${INSTALL_NEMOCLAW:-0}" == "1" ]]; then
  if ! have curl; then
    fail "curl is required to install NemoClaw."
  fi
  log "Installing NemoClaw. The installer may prompt for host setup."
  if [[ "${NEMOCLAW_NON_INTERACTIVE:-0}" == "1" ]]; then
